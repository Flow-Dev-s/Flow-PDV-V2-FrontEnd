...
import { Globe, Save, Server, Settings, X } from 'lucide-react'
...
function App() {
  ...
  localStorage.setItem('@FlowPDV:apiUrl', serverIp);
  ...
  const confirmAndSaveSale = async () => {
    try {
      ...
      await axios.post('${getApiUrl()}/api/sales', salePayload);
...
if (!currentUser) {
  ...
  {/* CUPOM NÃO-FISCAL (ESCONDIDO, SÓ APARECE NA IMPRESSÃO) */}
      ...
      {/* MODAL DE CONFIGURAÇÕES (IP DO SERVIDOR) */}
      {showSettings && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-[80] animate-in fade-in p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden flex flex-col animate-in zoom-in-95">
            ...
            <div className="p-5 border-b border-slate-100 flex items-center justify-between bg-slate-50">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-lg font-bold text-slate-800">Configurações de Rede</h2>
                  <p className="text-xs text-slate-500">Conexão com o Back-end</p>
                </div>
              </div>
              <button 
            ...
            <div className="p-6 space-y-4">
              <div className="bg-blue-50 text-blue-800 p-4 rounded-xl text-sm flex gap-3 border border-blue-100">
                <Globe size={24} className="shrink-0 text-blue-500" />
                <p>
                  Informe o IP da máquina onde o servidor (API) está rodando. 
                  Exemplos: <strong>http://localhost:8080</strong> ou <strong>http://192.168.1.15:8080</strong>.
                </p>
              </div>
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-11 pr-4 py-3 rounded-xl border-2 border-slate-200 bg-white text-slate-700 font-medium outline-none focus:border-blue-500 transition-colors"
                  />
                </div>
              </div>
            </div>
            ...
            <div className="p-4 border-t border-slate-100 bg-slate-50 flex justify-end gap-3">
              ...
              <button 
                ...
                className="px-5 py-2.5 rounded-xl font-medium bg-blue-600 text-white hover:bg-blue-700 transition-colors text-sm flex items-center gap-2 shadow-sm"
              >
                <Save size={16} />
                Salvar e Reiniciar
              </button>
